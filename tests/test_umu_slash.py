...
class TestFallback:
    ...
    async def test_create_course_fallback_to_admin(self, ctx):
        ...
        # 未指定 default_role，仅 admin 配置时直接选用，不算 fallback
...
class TestExecution:
    ...
    @pytest.mark.asyncio
    async def test_admin_role_resolves_course_management(self, ctx):
        ctx.mcp.list_servers.return_value = ["teacher", "admin"]
        ctx.mcp.call_tool = AsyncMock(
            return_value=ToolCallResult(success=True, data={"courses": []})
        )
        with patch(
            "umu_sdk.skills.slash._runner.get_configured_roles",
            return_value=["admin"],
