...
class TestTeacherCourseStudentLists:
    async def test_list_course_participants_forwards_arguments(
        self, registry_with_teacher_skills: SkillRegistry
    ) -> None:
        mock_mcp = MockMCPClientManager()
        skills_server._skill_registry = registry_with_teacher_skills
    ...
    async def test_list_course_participants_propagates_failure(
        self, registry_with_teacher_skills: SkillRegistry
    ) -> None:
        responses = {
            ("teacher", "tch_list_course_participants"): {
                "success": False,
        ...
        parsed = json.loads(result)
        assert parsed["success"] is False
        assert parsed["error_code"] == "LIST_COURSE_PARTICIPANTS_ERROR"
        assert parsed["error_message"] == "API 失败"
        assert parsed["next_action"] == "retry"

    async def test_list_course_learning_durations_forwards_arguments(
        self, registry_with_teacher_skills: SkillRegistry
    ) -> None:
        mock_mcp = MockMCPClientManager()
        skills_server._skill_registry = registry_with_teacher_skills
    ...
    async def test_list_course_learning_durations_propagates_failure(
        self, registry_with_teacher_skills: SkillRegistry
    ) -> None:
        responses = {
            ("teacher", "tch_list_course_learning_durations"): {
                "success": False,
        ...
        parsed = json.loads(result)
        assert parsed["success"] is False
        assert parsed["error_code"] == "LIST_COURSE_LEARNING_DURATIONS_ERROR"
        assert parsed["error_message"] == "API 失败"
        assert parsed["next_action"] == "retry"
