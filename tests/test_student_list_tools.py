...
def _patch_student_auth(require_auth: bool = False):
    ...
    stack = [
        patch("umu_sdk.adapters.mcp.student._get_client", return_value=client),
    ]
    if require_auth:
...
def _my_course_page(
    ...
    """构造 stu_get_my_courses 的分页响应."""
    return {
        "error_code": 0,
        "data": {
            "page_info": {"list_total_num": total},
            "list": items,
        },
    }
...
class TestStuGetMyCourses:
    async def test_fetch_all_two_pages_reuses_successful_endpoint(
        self, capsys: pytest.CaptureFixture[str]
    ) -> None:
        from umu_sdk.adapters.mcp.student import stu_get_my_courses

        page1 = _my_course_page(1, 50, 2, [{"group_id": "g1", "title": "Course 1"}])
        page2 = _my_course_page(2, 50, 2, [{"group_id": "g2", "title": "Course 2"}])

        with _patch_student_auth() as client:
            # 第一个端点成功，后续复用同一端点
            client.get.side_effect = [page1, page2]
            result = json.loads(await stu_get_my_courses(fetch_all=True))

        assert result["success"] is True
        assert len(result["data"]["courses"]) == 2
        output = capsys.readouterr().err
        assert "[stu_get_my_courses]" in output
    ...
    async def test_fetch_all_uses_fallback_endpoint(
        ...
        page = _my_course_page(1, 50, 1, [{"group_id": "g1", "title": "Course 1"}])
    ...
    async def test_single_page_no_progress(self, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        resp = _my_course_page(1, 20, 1, [{"group_id": "g1", "title": "Course 1"}])

        with _patch_student_auth() as client:
            client.get.return_value = resp
            result = json.loads(await stu_get_my_courses())

        assert result["success"] is True
        assert "[stu_get_my_courses]" not in capsys.readouterr().err
...
class TestStuListParticipatedCourses:
    async def test_fetch_all_two_pages(self, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        assert len(result["data"]["courses"]) == 2
        output = capsys.readouterr().err
        assert "[stu_list_participated_courses]" in output
        assert "获取完成" in output
