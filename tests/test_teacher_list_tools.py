...
class TestTchListCourseParticipants:
    async def test_single_page_maps_params_and_returns_students(self) -> None:
        from umu_sdk.adapters.mcp.teacher import tch_list_course_participants
        ...
        with _patch_teacher_auth(require_auth=True) as client:
            client.get.return_value = api_response
            result = json.loads(
                await tch_list_course_participants(
                    group_id="g123",
                    status_filter="completed",
                    include_disabled=False,
                    page=1,
        ...
        assert len(result["data"]["students"]) == 1
        assert result["data"]["students"][0]["user_name"] == "Alice"
        assert result["data"]["students"][0]["field_123"]["status"] == "2"
        call_args = client.get.call_args
        params = call_args.kwargs["params"]
        assert params["group_id"] == "g123"
        assert params["type"] == "1"
    ...
    async def test_fetch_all_merges_pages_and_reports_progress(
        ...
        page1 = {
            ...
            "error_code": 0,
            "data": {
                "data_count": {"total_num": 2, "complete_num": 2, "uncomplete_num": 0, "complete_rate": "1.00"},
                "table_body": {
                    "page_info": {"list_total_num": 2, "total_page_num": 2, "current_page": 1, "size": 50},
                    "list": [_student("s1", "Alice")],
                },
        ...
        with _patch_teacher_auth(require_auth=True) as client:
            client.get.side_effect = [page1, page2]
            result = json.loads(await tch_list_course_participants(group_id="g1", fetch_all=True))
...
class TestTchListCourseLearningDurations:
    async def test_single_page_maps_params_and_returns_students(self) -> None:
        from umu_sdk.adapters.mcp.teacher import tch_list_course_learning_durations
        ...
        with _patch_teacher_auth(require_auth=True) as client:
            client.get.return_value = api_response
            result = json.loads(
                await tch_list_course_learning_durations(
                    group_id="g123",
        ...
        assert result["success"] is True
        assert result["data"]["summary"]["avg_vlt"] == "00:18:05"
        assert len(result["data"]["students"]) == 1
        assert result["data"]["students"][0]["user_info"]["user_name"] == "Alice"
        assert result["data"]["students"][0]["field_123"]["vlt"] == "00:01:14"
        call_args = client.get.call_args
        params = call_args.kwargs["params"]
        assert params["group_id"] == "g123"
        assert params["type"] == "2"
        assert params["filter_disabled_user"] == "0"
        assert params["v"] == "2"
    ...
    async def test_fetch_all_merges_pages_and_reports_progress(
        ...
        def _student(sid: int, name: str) -> dict[str, Any]:
            return {
                "student_id": sid,
                "user_info": {"user_name": name, "umu_id": f"u{sid}"},
                "learning_time_for_course": {"vlt": "00:10:00", "sum_learning_time": "00:10:00"},
            }
        ...
        page1 = {
            ...
            "error_code": 0,
            "data": {
                "data_count": {"total_num": 2, "complete_num": 2, "uncomplete_num": 0, "complete_rate": "1.00", "avg_vlt": "00:10:00"},
                "table_body": {
                    "page_info": {"list_total_num": 2, "total_page_num": 2, "current_page": 1, "size": 50},
                    "list": [_student(1, "Alice")],
                },
            },
        }
        page2 = {
        ...
        with _patch_teacher_auth(require_auth=True) as client:
            client.get.side_effect = [page1, page2]
            result = json.loads(await tch_list_course_learning_durations(group_id="g1", fetch_all=True))
