...
- **高频流程封装 + 原子工具兜底**：93+ 个内置 Skill 覆盖常见跨角色流程，AI 优先通过 `skill_run` 语义化调用；低频或新增能力可通过 `skill_call_atomic_tool` 直接透传 149+ 个原子工具。
...
- **三种使用形态**：既可通过 `/umu` 对话式使用，也可作为 Python SDK 直接调用，还能以独立 MCP server 接入 Claude Desktop、Cursor、Cline 等客户端。
...
| 班级 | `adm_list_classes` |
| 课程/学习项目 | `adm_list_courses`, `adm_list_learning_programs`, `adm_list_personal_learning_programs`, `adm_set_course_access_permission`, `adm_get_course_access_permission`, `adm_get_course_access_list`, `adm_search_access_accounts`, `adm_add_course_access_accounts`, `adm_remove_course_access_accounts`, `adm_cancel_all_assigned_permissions`, `adm_set_course_auto_close`, `adm_cancel_course_auto_close`, `adm_set_program_access_permission`, `adm_get_program_access_permission`, `adm_get_program_access_list`, `adm_search_program_access_accounts`, `adm_add_program_access_accounts`, `adm_remove_program_access_accounts`, `adm_cancel_all_program_permissions` |
...
| `set_course_access_permission_admin` | admin | 设置课程访问权限（企业内公开/指定账户/班级/部门/分组/关闭） |
| `get_course_access_permission_admin` | admin | 获取课程当前访问权限设置 |
| `get_course_access_list_admin` | admin | 获取课程当前已授权的访问列表 |
| `search_course_access_accounts_admin` | admin | 搜索可授权访问课程的账户、班级、部门或分组 |
| `add_course_access_accounts_admin` | admin | 为课程设置指定账户、班级、部门或分组的访问权限 |
| `remove_course_access_accounts_admin` | admin | 移除课程的指定账户、班级、部门或分组访问权限 |
| `cancel_course_access_permissions_admin` | admin | 取消课程的所有指定访问权限 |
...
| `set_program_access_permission_admin` | admin | 设置学习项目访问权限 |
| `get_program_access_permission_admin` | admin | 获取学习项目当前访问权限设置 |
| `get_program_access_list_admin` | admin | 获取学习项目当前已授权的访问列表 |
| `search_program_access_accounts_admin` | admin | 搜索可授权访问学习项目的账户、班级、部门或分组 |
| `add_program_access_accounts_admin` | admin | 为学习项目设置指定账户、班级、部门或分组的访问权限 |
| `remove_program_access_accounts_admin` | admin | 移除学习项目的指定账户、班级、部门或分组访问权限 |
| `cancel_program_access_permissions_admin` | admin | 取消学习项目的所有指定访问权限 |
