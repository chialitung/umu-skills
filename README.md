...
- **高频流程封装 + 原子工具兜底**：105+ 个内置 Skill 覆盖常见跨角色流程，AI 优先通过 `skill_run` 语义化调用；低频或新增能力可通过 `skill_call_atomic_tool` 直接透传 149+ 个原子工具。
...
内置 Skill 覆盖高频场景（共 105），并支持通过 `/umu`、`/umua`、`/umut`、`/umus` 斜杠命令直接触发：
