...
def _parse_admin_response(resp: dict[str, Any]) -> tuple[bool, Any, str]:
    """解析 UMU 管理员接口响应.
...
def _search_access_permission_account(
    ...
    ok, result, err = _parse_admin_response(resp)
    if not ok:
        return False, [], err
...
def _set_obj_access_permission(
    ...
    ok, data, err = _parse_admin_response(resp)
    if not ok:
...
@mcp.tool()
async def adm_set_course_access_permission(
    ...
    - 0：关闭，任何人均不可访问
    - 2：企业内公开，企业内成员均可访问
    - 3：指定账户/班级/部门/分组可见，需要再调用 adm_add_course_access_accounts 添加可见对象
    ...
    try:
        ...
        return _ok(
            ...
            next_action="proceed",
            suggested_action=(
                "access_permission=3 时，请继续调用 adm_add_course_access_accounts 添加可见账户"
...
@mcp.tool()
async def adm_get_course_access_permission(
    group_id: Annotated[str, Field(description="课程 ID")],
    session_id: Annotated[
        str | None,
        Field(
...
@mcp.tool()
async def adm_get_course_access_list(
    group_id: Annotated[str, Field(description="课程 ID")],
...
@mcp.tool()
async def adm_search_access_accounts(
    group_id: Annotated[str, Field(description="课程 ID")],
    ...
    try:
        ...
        return _ok(
            ...
            next_action="proceed",
            suggested_action="请从结果中选择目标账户/班级，调用 adm_add_course_access_accounts 添加权限",
        )
    except Exception as e:
...
@mcp.tool()
async def adm_add_course_access_accounts(
    group_id: Annotated[str, Field(description="课程 ID")],
    accounts: Annotated[
        list[dict[str, Any]],
        Field(
            description="要添加的账户/班级/部门/分组列表，每个元素需包含 account、account_type(user/class/department/group)、id，"
                        ...
                        "group 类型还需包含 user_group_id（可调用 adm_search_access_accounts 获取）",
        ),
    ],
    session_id: Annotated[
        str | None,
    ...
    if not accounts:
        return _err(
            ...
            "未提供要添加的账户/班级",
            suggested_action="请先调用 adm_search_access_accounts 查询目标",
    ...
    try:
        ...
        return _ok(
            ...
            next_action="proceed",
            suggested_action="可调用 adm_search_access_accounts 或查看 UMU 后台确认权限已生效",
        )
    except Exception as e:
...
@mcp.tool()
async def adm_remove_course_access_accounts(
    group_id: Annotated[str, Field(description="课程 ID")],
    accounts: Annotated[
        list[dict[str, Any]],
        Field(
...
@mcp.tool()
async def adm_cancel_all_assigned_permissions(
    group_id: Annotated[str, Field(description="课程 ID")],
    session_id: Annotated[
        str | None,
        Field(
    ...
    如需同时设置为企业内公开，请在调用后继续调用 adm_set_course_access_permission。
    """
    client = _get_client(session_id)
    auth_err = _require_auth(client)
    if auth_err:
    ...
    try:
        ...
        return _ok(
            ...
            next_action="proceed",
            suggested_action="如需还原为企业内公开，请调用 adm_set_course_access_permission(group_id, access_permission=2)",
        )
    except Exception as e:
