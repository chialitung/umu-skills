...
【小节类型速查】
list_sections 返回的每个小节包含 type 字段：
- type="scorm" → SCORM 小节（sessionType=11）→ 用 tch_update_scorm_section 修改
- type="document" → 文档小节（sessionType=14）→ 用 tch_update_document_section 修改
- type="infographic" → 图文小节（sessionType=15）→ 用 tch_update_infographic_section 修改，用 tch_get_infographic_content 查看内容
- type="exam" → 考试小节（sessionType=10）→ 用 tch_update_exam_section 修改
- type="signin" → 签到小节（sessionType=6）→ 用 tch_update_signin_section 修改
- 其他类型 → 暂不支持修改，可删除后重新创建
