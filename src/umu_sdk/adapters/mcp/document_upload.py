...
封装"我的文档"资源上传的完整流程，支持：
- Excel (.xlsx), Word (.docx), PowerPoint (.pptx), PDF (.pdf) 等格式
- 小文件直接 PUT 上传
- 大文件分片并发上传（复用 ScormUploader 的 COS 机制）
- 自动注册到文档资源列表

与 ScormUploader 的区别：
- media_type 为 "docweike"（非 "videoweike"）
- 不需要 SCORM 解析注册和状态轮询
- 回调 endpoint 为 /microapi/resourcemgt/resourceCallback
"""
...
def validate_document_path(file_path: str) -> str:
    ...
    if ext not in SUPPORTED_DOC_EXTENSIONS:
        raise ValueError(
            f"不支持的文档格式 '{ext}'。支持的格式: {', '.join(sorted(SUPPORTED_DOC_EXTENSIONS))}"
        )
...
class DocumentUploader:
    ...
    阶段：
        ...
        4. add_log — 记录上传日志
        5. callback — resourceCallback 注册到文档列表
    ...
    def stage_1_validate(self, file_path: str, name: str | None = None) -> None:
        """验证文件并初始化上传器状态."""
        logger.info("[Stage 1] 验证文档: %s", file_path)
        ...
        logger.info(
            "[Stage 1] 文档验证通过: %s, size=%d bytes, ext=%s",
            self.file_name,
            self.file_size,
            self.file_ext,
        )
    ...
    def stage_2_get_credentials(self) -> COSCredentials:
        ...
        pre_payload = {
            "data": (
                '{"opts":['
                f'{{"key":"media_type","value":"{DOC_MEDIA_TYPE}"}},'
    ...
    async def _upload_single(self) -> None:
        ...
        content_type = _get_content_type(self.file_ext)
        ...
        try:
            self.client.get(
                ...
                params={
                    ...
                    "refer": self.client.desktop_url("/documentDrive"),
                    "action_type": "upload",
                    "resource_type": "1",
                    "origin_type": "2",
                    "device_type": "1",
    ...
    def stage_5_callback(self) -> dict[str, Any]:
        """调用 resourceCallback 将资源注册到文档列表.

        这是资源能在前端"我的文档"页面中显示的关键步骤。

        Returns:
            callback 响应数据
        """
        logger.info("[Stage 5] 注册资源到文档列表...")
