...
name: umu-student
description: |
  当用户输入 /umus 或 /umustudent 斜杠命令时触发。
  固定以 Student（学员）角色执行 UMU 平台操作。
  覆盖课程报名、浏览课程、完成小节（视频、文章、问卷、考试、签到）、学习进度跟踪等全部学员场景。
---

# /umus — UMU 学员操作助手

本 skill 指导 Claude 以学员身份通过 MCP 工具操作 UMU（优幕）学习平台。

## 触发条件

以下情况必须调用本 skill：

1. 用户输入 `/umus` 或 `/umustudent`。
...
- Skill 文件位置：`~/.claude/skills/umu-student/`
- 加密方式：Fernet 对称加密
...
3. **批量查询明细**：对于"查询我所有课程的学习进度""查询我所有未完成的考试"等需要对每门课程再调用的任务，不要逐个调用原子工具。优先派遣子代理批量处理，子代理可直接使用 `umu_sdk` 的 `UMUClient` 底层接口高效拉取。
4. **导出 Excel/CSV 优先使用专用导出工具**：学员端暂无专用导出工具，未来补充。如果存在 `stu_export_*` 工具则直接调用。
5. **无专用工具时兜底导出**：派遣子代理完成：子代理拉取全量数据后，优先使用 `ExportEngine.export_records()` 生成 Excel/CSV，最后才用 pandas/openpyxl 手动实现，保存到用户指定路径（未指定则默认桌面）。
