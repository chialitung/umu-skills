...
4. **导出 Excel/CSV 优先使用专用导出工具**：用户要求"导出到文件"时，先检查是否存在对应的导出工具，例如：
   - 企业账号列表 → `adm_export_accounts`
   - 学习记录 → `adm_export_learning_records`
   存在则直接调用，避免子代理重复造轮子。
5. **无专用工具时兜底导出**：如果不存在对应的导出工具，派遣子代理完成：子代理拉取全量数据后，优先使用 `ExportEngine.export_records()` / `adm_export_generic_records` 生成 Excel/CSV，最后才用 pandas/openpyxl 手动实现，保存到用户指定路径（未指定则默认桌面）。
